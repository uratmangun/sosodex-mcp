...
# Record the video (headless; best for CI)
npx webreel record my-demo

# Record with visible Chrome (watch the run while capturing MP4)
# sosodex-mcp: pnpm run demo:record
WEBREEL_HEADED=1 node scripts/webreel/record-with-preview.mjs my-demo --verbose
```
